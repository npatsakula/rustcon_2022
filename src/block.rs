...
impl<'input, V: Copy> BlockExpression<'input, V> {
    ...
    pub fn nodes_count(&self) -> usize {
        self.inner.as_ref().collapse_layers(|node| match node {
            Layer::Value(_) => 1usize,
            Layer::Op { left, right, .. } => left + 1 + right,
            Layer::FnCall { arguments, .. } => 1 + arguments.into_iter().sum::<usize>(),
        })
    }
}
...
mod block_tests {
    ...
    proptest::proptest! {
        #[test]
        fn expression_equality(source in crate::properties::expression().prop_filter("must be normal", |e| e.evaluate().is_normal())) {
            let evaluated = source.evaluate();
            let nodes = source.leafs_count();

            let block = TopBlockExpression::new(source);
            prop_assert_eq!(evaluated, block.evaluate());
            prop_assert_eq!(nodes, block.nodes_count());
        }
    }
}
