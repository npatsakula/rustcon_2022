...
pub(crate) fn function_call<S: Strategy<Value = V> + Clone + 'static, V: Clone + Debug + 'static>(
    value_strategy: S,
    user_defined_functions: Vec<Arc<UserDefinedFunction<'static, 'static>>>,
...
pub(crate) fn functions() -> impl Strategy<Value = Vec<Arc<UserDefinedFunction<'static, 'static>>>> {
...
pub fn top_level_expression_with_functions() -> impl Strategy<
    Value = (
        Vec<Arc<UserDefinedFunction<'static, 'static>>>,
        Expression<'static, 'static, f64>,
    ),
> {
    ...
    functions().prop_flat_map(|user_defined_functions| {
        expression(value(), user_defined_functions.clone())
            .prop_map(move |e| (user_defined_functions.clone(), e))
    })
}

pub fn top_level_expression() -> impl Strategy<Value = Expression<'static, 'static, f64>> {
    top_level_expression_with_functions().prop_map(|(_functions, expr)| expr)
}
