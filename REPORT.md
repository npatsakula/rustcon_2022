...
В качестве парсер-генератора мы возьмём LALRPOP [1]; для описания грамматики он использует свой язык,
основы которого я приведу ниже.
...
реализовывать её будем через генерацию заведомо корректных деревьев при помощи `proptest` [2],
...
[1] LALRPOP book: [оригинал](http://lalrpop.github.io/lalrpop/).
[2] Proptest book: [оригинал](https://altsysrq.github.io/proptest-book/intro.html).
...
[1] Criterion book: [оригинал](https://bheisler.github.io/criterion.rs/book/index.html);

## Pantano: cache locality

Причиной, в силу которой в ходе вычисления мы половину времени ожидаем ~~чуда~~,
...
представляется однородной, хотя по факту она таковой не является [1]: процессор имеет
...
* Так как дерево вычисляется строго от листа к корню, мы можем избавиться от рекурсии:
  ...
  3. Тогда вместе с вектором вычислений это будет выглядеть как:
     ...
     использовали для хранения выражения (в нашем коде мы оперируем выражением `Value(f64)`).
   Мы могли бы проделать всю эту машинерию самостоятельно, но она уже реализована в составе
   крейта `recursion` [2]. В силу того, что я не могу опубликовать свою реализацию этой идеи,
...
[оригинал](https://people.freebsd.org/~lstewart/articles/cpumemory.pdf).

[2] Elegant and performance recursion in Rust, Inanna Malick:
[оригинал](https://recursion.wtf/posts/rust_schemes/).
...
С не владеющими строками всё несколько сложнее, т.к. они ломают наш код сразу в
нескольких местах:
...
Чтобы понять, что это такое и зачем оно нам нужно, давайте разберёмся, как вычисляется
функция вида:
...
instruction или чем-то в таком духе.

Что же, остаётся только замерять производительность. Для этого сгенерим тысячу функций
с большой вложенностью, скомпилируем и сравним скорость вычисления с обычной:
...
Полученные результаты гарантировано можно улучшить тюнингом набора оптимизаций,
которые применяет LLVM. Тем не менее, уже сейчас мы превзошли наивное вычисление
результата.

## El epílogo

В послесловии хотелось бы оставить не сухой анализ, а ощущения, которые возникли
при написании этой работы.

1. У языка предостаточно инструментов для парсинга структурированныз данных. Лишь
   некоторые из них я могу рекомендовать к промышленному использованию, но многие
   приятно удивят даже искушённых традиционно сильными в этой стезе языками, как Haskell.
2. Инструментарий по получению производительности отлично укладывается в правило
   Парето: основные места, где можно скостить такты, устраняются легко (много
   эффективных структур данных, легко подключается более производительный аллокатор,
   чаще всего легко найти узкие места); оставшиеся двадцать процентов проходят
   тяжелее, чем у условного С++ (лайфтаймы делают больно, разработка кросс-платформенных
   SIMD стагнирует, аллокаторный интерфейс тоже не в ближайших планах).
3. Общедоступный инструментарий для property-based тестирования и JIT компиляции уже
   дозрел до того уровня, когда можно без невыносимой боли и за вменяемые сроки
   использовать его в промышленной разработке, не обладая специальными знаниями
   в компиляторостроении или ассемблере целевой платформы.

Я надеюсь, что описанная здесь история оказалось полезной и побудит читателя на
