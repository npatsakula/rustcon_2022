...
1. Гарантии корректности: если речь идёт про свободные от контекста грамматики, мы хотим, чтобы лексемы и структуры разбирались однозначным
   ...
   fn not(source: &str) -> IResult<&str, Query> {
       tag("-").and_then(query).parse(source)
   }
   ...
   ```
   LALRPOP же отказывает компилировать следующий код мотивируя это тем, что `And` и `Or` --
   одно и то же:
   ```lalrpop
   ...
   AndLeaf = { Value, "(" <Or> ")", Not };
   And: Query = Many2<AndLeaf, "&"> => Query::And(<>);
   
   OrLeaf = { Value, "(" <And> ")", Not };
   Or: Query = Many2<OrLeaf, "|"> => Query::Or(<>);
...
fn tripple(fst, snd, thrd) = double(fst, snd) * thrd;

cos(tripple(2.0, constant() * pi(), 8.0 / 2.0))
```
...
2. Функции.
    ...
    2. Определяемые пользователем: с глобальной областью видимости **уже определённых** функций, без
       форсирования вычислений (ползволит экспериментировать с ленивыми вычислениями).
...
Генерализация нужна нам для того, чтобы на уровне типов выразить ограничения на то, что может является элементом
...
Генерация данных для парсинга идёт обратной ему дорогой, потому нам нужно сгенерировать часть
...
вычисления. В этой связи есть два соображения:
...
1. Локальности измерений: делая оценку производительности вычислителя, мы не
   хотим померять что-то ещё (например, десериализацию данных), потому утилита
...
4. Время аллокации занимает всего 4% времени, смысла оптимизировать этот участок при
   помощи кэширующего аллокатора или арены не много.
...
2. В имеющимся `flamegraph` нет функции, за которую можно зацепиться взглядом, кроме как
...
| Cache type | Acess ticks | Cache size        |
| ---------- | ----------- | ----------------- |
| Register   | 0           | dozens            |
| L1         | 4           | 64 KB (per core)  |
...
1. Сделать наполнение одной cache-line максимально полным, чтобы процессору не приходилось
   доставать извлекать из кэша новую линию и для того, чтобы реже происходило вытеснение
   из кэша полезных данных.
...
за счёт чего мы проваливаемся сразу по двум пунктам.
1. За счёт того, что `Expression<f64>` занимает всего 40 байт, мы оставляем ещё
   24 байта бесхозными.
2. За счёт того, что что аллокатор уложит эту память не последовательно, мы не
   можем префетчить память заблаговременно.
